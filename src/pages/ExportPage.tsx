...
import { Download, FileSpreadsheet, Filter, Calendar, X, CheckCircle } from 'lucide-react';
...
const ExportPage: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    const exportData = filteredPayments.map((payment, index) => ({
      ...
      'Bill Date': format(new Date(payment.billDate), 'dd/MM/yyyy'),
      'Requested By': payment.requestedBy.name,
      'Approved By': payment.approvedBy ? payment.approvedBy.name : 'N/A',
    ...
    XLSX.writeFile(workbook, `Payment_Report_${today}.xlsx`);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Export payment data to Excel for processing
        </p>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['pending', 'approved', 'rejected', 'processed'].map(statusOption => (
                  <button
                    key={statusOption}
